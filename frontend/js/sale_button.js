...
async function updateSales(drink, quantity) {
    const salesData = { [drink]: quantity };
    await fetch('/api/sales', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify(salesData),
    });
    console.log(`Sales updated for ${drink}: +${quantity}`);
}

document.querySelectorAll('.sale-button').forEach((button) => {
    button.addEventListener('click', async (event) => {
        const drink = event.target.dataset.drink;
        const quantity = 1; // Assume 1 sale per click
        await updateSales(drink, quantity);
    });
});
