...
class Prices{
    ...
    async set_default(){
        const default_normal_prices = await fetchPricesFromAPI();
        for (let drink in default_normal_prices) {
            this.prices_history[drink] = [default_normal_prices[drink].price];
        }
    ...
    compute_new_prices(new_sales, indexes, default_prices) {
        ...
        for (let drink in this.prices_history) {
            ...
            new_prices[drink] = this.round_to_nearest_5_cents(Math.max(
    ...
    // Helper function to round to the nearest 5 cents
    ...
    async get_sales() {
        ...
        for (let drink in prices) {
            sales[drink] = prices[drink].sales;
        }
        return sales;
    }
}
